...
@section('content')
    ...
    <div
        ...
        <div
            ...
            <h1 class="col-start-1 font-black text-2xl text-black">Projet : {{ $projet->libelle }} - {{ $domaine->libelle }}
            </h1>
                    ...
                    @else
                        @foreach ($etapes as $etape)
                            <tr class="text-neutral-800">
                                ...
                                <td class="px-5 py-4 text-sm font-medium text-right whitespace-nowrap">
                                    @if ($etape->facture)
                                    <a href="{{ route('chef.projet.etape.show', ['etape' => $etape->id]) }}"
...
<?php
    $total = $intervenants->count();
    if ($total > 0) {
        $prestataires = $intervenants->where('prestataire', true)->count();
        $salaries = $intervenants->where('prestataire', false)->count();
    
    
        $prestatairesPercent = ($prestataires / $total) * 100;
        $salariesPercent = ($salaries / $total) * 100;
...
?>
            ...
            <div class="row-start-2 h-fit border-2 border-neutral-400 rounded-xl m-4">
                ...
                <table class="min-w-full divide-y divide-neutral-700">
                    ...
                    <tbody class="divide-y divide-neutral-200">
                        @if (!$intervenants->where('prestataire', false)->count() > 0)
                            <tr class="text-neutral-800">
                                <td class="px-5 py-4 text-sm font-medium whitespace-nowrap">Aucune donnée</td>
                                <td class="px-5 py-4 text-sm font-medium text-right whitespace-nowrap">
                                    <a class="text-blue-600 underline hover:text-blue-700 hover:cursor-not-allowed">Détails</a>
                        ...
                        @else
                            @foreach ($intervenants->where('prestataire', false) as $intervenant)
                                <tr class="text-neutral-800">
                                    <td class="px-5 py-4 text-sm font-medium whitespace-nowrap">{{ $intervenant->nom.' '.$intervenant->prenom }}</td>
                                    <td class="px-5 py-4 text-sm font-medium whitespace-nowrap text-right">{{ $intervenant->intervention->commentaire ?? '' }}</td>
                                </tr>
                            @endforeach
                        @endif
                    </tbody>
            ...
            <div class="row-start-2 h-fit border-2 border-neutral-400 m-4 rounded-xl">
                ...
                <table class="min-w-full divide-y divide-neutral-700">
                    ...
                    <tbody class="divide-y divide-neutral-200">
                        @if (!$intervenants->where('prestataire', true)->count() > 0)
                            <tr class="text-neutral-800">
                                <td class="px-5 py-4 text-sm font-medium whitespace-nowrap">Aucune donnée</td>
                                <td class="px-5 py-4 text-sm font-medium whitespace-nowrap">Aucune donnée</td>
                                <td class="px-5 py-4 text-sm font-medium whitespace-nowrap">Aucune donnée</td>
                        ...
                        @else
                            @foreach ($intervenants->where('prestataire', true) as $intervenant)
                                <tr class="text-neutral-800">
                                    <td class="px-5 py-4 text-sm font-medium whitespace-nowrap">{{ $intervenant->getPrestataire->raison_sociale }}</td>
                                    <td class="px-5 py-4 text-sm font-medium whitespace-nowrap">{{-- $intervenant->getPrestataire->raison_sociale --}} 73.5€</td>
                                    <td class="px-5 py-4 text-sm font-medium text-right whitespace-nowrap">{{ $intervenant->intervention->commentaire ?? '' }}</td>
                                </tr>
                            @endforeach
                        @endif
                    </tbody>
    ...
    <dialog id="addStep" class="min-w-96 w-3/4 h-fit rounded relative overflow-hidden p-8 space-y-4">
        ...
        <form action="{{ route('chef.projet.etape.add', ['projet' => $projet->id]) }}" method="POST"
