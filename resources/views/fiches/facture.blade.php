...
<div class="h-full p-4 justify-self-stretch bg-gray-400 rounded-md row-start-2 col-start-2 flex flex-col items-stretch gap-4">
    <form method="POST" class="flex flex-col items-start gap-2 rounded-md border-2 border-neutral-700 bg-neutral-100 p-4">
        <h1 class="col-start-1 font-black text-2xl text-black">Facturer le client : {{ $etape->projet->client->raison_sociale }}</h1>
        <h3 class="col-start-1 font-bold text-xl text-black">Pour l'étape : {{ $etape->libelle }}</h3>
        <h4 class="col-start-1 font-semi bold text-md text-black">Du projet : {{ $etape->projet->libelle }} ( {{ $etape->projet->taux_horaire }}€ /H )</h4>
        ...
        <div class="flex flex-row gap-8 w-full">
            ...
            <div class="flex flex-col gap-2 w-full">
                ...
                <input type="datetime-local" {{ $etape->facture ? 'disabled' : '' }} name="date_facture" id="date" class="w-full rounded-md border-2 border-neutral-700 bg-neutral-100 p-2" value="{{ old('date') ?? now()->format('Y-m-d H:i') }}" step="1800">
            </div>
        ...
        <div class="relative overflow-x-auto shadow-md rounded-lg w-full mt-8">
            ...
            <div class="w-full h-fit p-2 bg-white flex flex-row">
                <div class="flex flex-col">
                    ...
                    <h4>Total facturé : {{ $etape->projet->total }} €</h4>
                </div>
